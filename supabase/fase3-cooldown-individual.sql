-- =====================================================================
-- Pandapp Family — fase 3: cooldown personal per a tasques individuals
-- =====================================================================
-- Migració incremental per a la base de dades real. Segura de tornar a
-- executar sencera (ADD COLUMN IF NOT EXISTS, CREATE OR REPLACE, UPDATE
-- idempotent).
--
-- Motiu: "Fer el llit", "Escombrar o aspirar una estança" i "Ordenar la
-- teva habitació" tenien el cooldown per defecte (de la tasca, no de la
-- persona): que algú fes la seva bloquejava la tasca per a tothom, quan
-- cadascú té el seu propi llit/habitació. Ara es pot marcar una activitat
-- com `cooldown_individual` perquè cada usuari tingui el seu propi
-- cooldown (i la seva pròpia escalada per oblit, que en depèn).
-- =====================================================================

alter table public.activitats
  add column if not exists cooldown_individual boolean not null default false;

comment on column public.activitats.cooldown_individual is
  'false (per defecte): cooldown de la tasca, bloqueja tothom (recurs compartit). true: cada usuari té el seu propi cooldown i la seva pròpia escalada per oblit.';
