-- =====================================================================
-- Pandapp Family — fase 2, peça 1: escalada per oblit
-- =====================================================================
-- Migració incremental per a la base de dades real (ja no es pot rellançar
-- tot `schema.sql` perquè les taules ja existeixen). Reemplaça únicament
-- `reclamar_activitat` per una versió que calcula l'escalada per oblit.
-- Segur de tornar a executar (CREATE OR REPLACE).
--
-- Formula (veure CLAUDE.md "Sistema de punts → Escalada per oblit"):
--   punts = punts_base × (1 + k_dia × dies_passats_del_periode_normal)
--   limitat per `sostre`, arrodoniment normal (no cap amunt).
-- Si l'activitat encara no ha passat `periode_normal_h` des de l'última
-- completació (o no s'ha fet mai), és punts_base pla. Les activitats
-- personals no escalen perquè sempre tenen k_dia = 0 al catàleg: no cal
-- cap cas especial a la fórmula per a elles.
--
-- El resultat es guarda a `completions.punts_base_snapshot` (deixa de ser
-- `activitats.punts_base` pla) i és el que reben `pot_total` i
-- `participacions.punts_assignats`.
-- =====================================================================
