-- =====================================================================
-- Pandapp Family — fase 3: la foto passa a ser sempre opcional
-- =====================================================================
-- Migració incremental per a la base de dades real. Segura de tornar a
-- executar sencera (CREATE OR REPLACE).
--
-- Fins ara `reclamar_activitat` rebutjava la reclamació si l'activitat
-- tenia `requereix_foto = true` i no arribava cap `p_foto_url`. Ara la
-- foto sempre és opcional per a qui reclama, independentment del valor
-- de `requereix_foto` (que es manté a la taula per si es fa servir més
-- endavant per suggerir quan convé posar-ne, però ja no bloqueja res).
-- =====================================================================
